...
// Função para processar arquivo de transações - COM LOGS DETALHADOS
const parseTransactionsFile = (buffer) => {
  try {
    ...
    const validRows = rows.filter((row, index) => {
      const hasCustomerId = row[0] && String(row[0]).trim() !== '';
      const hasDate = row[1] && row[1] !== '';
      
      if (!hasCustomerId || !hasDate) {
        console.log(`⚠️ Linha ${index + 1} rejeitada: customer_id=${row[0]}, date=${row[1]}`);
      }
    ...
    const transactions = validRows.map((row, index) => {
      try {
        ...
        const ggr = Number(row[2]) || 0;
        const chargeback = Number(row[3]) || 0;
        const deposit = Number(row[4]) || 0;
        const withdrawal = Number(row[5]) || 0;
...
// Função para processar arquivo de pagamentos - COM LOGS DETALHADOS
const parsePaymentsFile = (buffer) => {
  try {
    ...
    // Filtro mais flexível
    const validRows = rows.filter((row, index) => {
      const hasAfiliadoId = row[1] && String(row[1]).trim() !== '';
      const hasDate = row[2] && row[2] !== '';
      
      if (!hasAfiliadoId || !hasDate) {
        console.log(`⚠️ Linha ${index + 1} rejeitada: afiliados_id=${row[1]}, date=${row[2]}`);
      }
    ...
    const payments = validRows.map((row, index) => {
      try {
        const clientes_id = row[0] ? String(row[0]).trim() : null;
        const afiliados_id = String(row[1]).trim();
        const date = new Date(row[2]);
        const value = Number(row[3]) || 0;
        const method = String(row[4] || 'cpa').trim();
        const status = String(row[5] || 'finish').trim();
        const classification = String(row[6] || 'normal').trim();
...
// Rota para importar transações - COM LOGS DETALHADOS
...
// Rota para importar pagamentos - COM LOGS DETALHADOS
...
const server = app.listen(port, '0.0.0.0', () => {
  console.log('=================================');
  console.log('🚀 SERVIDOR EXPRESS DEBUG INICIADO!');
  console.log('=================================');
  ...
  console.log('✅ IMPORTAÇÃO DE PLANILHAS COM LOGS DETALHADOS!');
  console.log('=================================');
});
